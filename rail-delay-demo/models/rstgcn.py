...
class SpatialAttentionDomain(nn.Module):
    ...
    def forward(self):
        ...
        mask = (self.A>0).float()
        S = S * mask + (-1e4)*(1-mask)   # block non-edges
        W = torch.softmax(S, dim=-1)     # row-stochastic
        return W
...
class GCNBlock(nn.Module):
    ...
    def forward(self, x, W):
        ...
        x = self.theta(x)            # (B,T,N,C)
        x = x.permute(0,1,3,2)       # (B,T,C,N)
        x = x.reshape(B*T, C, N)
        Wb = W.unsqueeze(0).repeat(B*T,1,1)
        x = torch.bmm(x, Wb.transpose(1,2))  # (B*T,C,N)
        x = x.reshape(B,T,C,N).permute(0,2,1,3)  # (B,C,T,N)
        return x
...
class Branch(nn.Module):
    ...
    def forward(self, x):          # (B,F,T,N)
        ...
        x = self.gcn(x, W)
        x = self.c2d(x)
        y = self.head(x)[:, :, -1, :]  # (B,1,N) lấy timestep cuối của nhánh
...
class RSTGCN(nn.Module):
    ...
    def forward(self, Xh, Xd, Xw):
        ...
        return F.relu(y)  # non-negative delays
